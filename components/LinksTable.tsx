...
import { removeProtocol, withComma, errorMessage } from "../utils/globals";
import { useStoreActions, useStoreState } from "../store";
import { Link as LinkType, Links as LinksType } from "../store/links";
...
interface RowProps {
  index: number;
  link: LinkType;
  setDeleteModal: (arg0: number) => void;
}
...
const data = await queries.link.get({ user_id: { eq: "1234" } }, { limit: 10 });
const LinksTable: FC = () => {
  const isAdmin = true; //useStoreState(s => s.auth.isAdmin);
  // @ts-ignore
  ...
  //TODO
  /*useEffect(() => {
    get(options).catch((err: { response: { data: { error: any; }; }; }) =>
    ...
    <Col width={1200} maxWidth="95%" margin="40px 0 120px" my={6}>
      ...
      <Table scrollWidth="800px">
        ...
        {!links.items.length ? (
          ...
          <>
            {/*@ts-ignore*/}
            {links.items.map((link: LinkType, index: number) => (
              <Row
