...
/**
 * @brief Render a Gallery
 * @description Render a gallery of images, the images are loaded from newData.files
 * @param param0 
 * @returns 
 */
function RenderGallery({newData}: ImageProps) {
  ...
  const getRenderGallery = (files: object[] | string[]) => {
    try {
  ...
  useEffect(() => {
    ...
    if (newData.files?.length) {
      getRenderGallery(newData.files);
    } else {
      forceUpdate();
    }
