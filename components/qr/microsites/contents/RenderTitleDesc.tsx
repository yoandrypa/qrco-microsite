...
export default function RenderTitleDesc({data, stylesData}: CustomProps) {
  ...
  return <Box sx={{ my: '10px', px: 2, textAlign: 'center' }}>
    {data.titleAbout && (
      <Typography sx={{color: theme => theme.palette.primary.main, ...handleFont(stylesData, 'T')}}>
        {data.titleAbout}
      </Typography>
    )}
    {data.descriptionAbout && (
      <Typography sx={{color: theme => theme.palette.primary.main, ...handleFont(stylesData,'S' )}}>
        {data.descriptionAbout}
      </Typography>
    )}
  </Box>;
