...
const QrWizard = ({children}: QrWizardProps) => {
  ...
  const handleNext = async () => {
    ...
    if (step === 0 && Boolean(data.isDynamic) && !Boolean(userInfo)) {
      ...
      // @ts-ignore
      const model = {...data, qrType: selected, userId: userInfo.attributes.sub, id: qrId, qrOptionsId: qrDesignId};
      ...
      if (data?.isDynamic) {
        ...
        const targetUrl = generateShortLink(`qr/${qrId}`);
        shortLink = await handleShort(targetUrl);
        // @ts-ignore
        model.shortLinkId = shortLink.id;
      }

      await createMainDesign(options);
      // @ts-ignore
      await QrHandler.create(model);
