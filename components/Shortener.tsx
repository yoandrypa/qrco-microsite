...
import { removeProtocol } from "../utils/globals";
...
const Shortener = () => {
  ...
  const [formState, { raw, password, text, select, label }] = useFormState<Form>(
    { showAdvanced: false },
    {
      withIds: true,
  ...
  const submitLink = async (reCaptchaToken?: string) => {
    try {
      const link = null; //TODO await submit({ ...formState.values, reCaptchaToken });
      setLink(link);
      formState.clear();
    } catch (err) {
      setMessage(
